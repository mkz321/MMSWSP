...
class ModelRunner():
        ...
        for X, Y in self.data_gen.get_batches(self.data_gen.train_set[0], self.data_gen.train_set[1], self.args.batch_size, True):
            ...
            if self.data_gen.normalize_pattern == 0:
                ...
                if self.data_gen.mode == "immediate":
                    ...
                    predict = output * new_stds + new_means
                    test = Y * new_stds + new_means
                    # print("啊啊啊val_CC:", predict)
                    yy = predict[:, :, :1]
                    y = test[:, :, :1]
                    #print("啊啊啊val_CC:", yy)
                    loss = self.criterion(yy, y)
    ...
    def evaluate(self, mode='valid'):
        ...
        for i in range(0, pre.shape[0]):
            diffXXBar = pre[i] - xBar
            diffYYBar = tes[i] - yBar
            SSR += (diffXXBar * diffYYBar)
            varX += diffXXBar ** 2
            varY += diffYYBar ** 2
        # print(type(varX),type(varY))
        x1 = torch.tensor(varX)
        y1 = torch.tensor(varY)
        # print(type(x),type(y))
        SST = np.sqrt(x1.cpu() * y1.cpu())
        # SSR = torch.tensor(SSR)
        # print(SSR,SST,type(SSR),type(SST))
        xxx = SSR.cpu() / SST
        #print("11111111111111val_CC", xxx)

        #if mode == 'test':
        #    self.predict = predict.data.cpu().numpy()
        #    self.ground_truth = test.data.cpu().numpy()

        return mse, rse, mae, xxx, pre, tes
    ...
    def run(self):
        ...
        try:
            for epoch in range(1, self.args.epochs+1):
                ...
                val_mse, val_rse, val_mae, val_xxx, vppre, vttes = self.evaluate()
                if val_mse < best_valid_mse:
                    best_valid_mse = val_mse
                    best_valid_rse = val_rse
                    best_valid_mae = val_mae
                    #print("11111111111111val_CC", val_xxx)

                self.optim.updateLearningRate(val_mse, epoch)

                test_mse, test_rse, test_mae, test_xxx, ppre, ttes = self.evaluate(mode='test')
                if test_mse < best_test_mse:
                    best_test_mse = test_mse
                    best_test_rse = test_rse
                    best_test_mae = test_mae
                    print("2222222222val_CC", test_xxx)

                    x1 = torch.tensor(ttes)
                    y1 = torch.tensor(ppre)
                    true_list = np.array(x1.cpu(), dtype='float64')
                    predict_list = np.array(y1.cpu(), dtype='float64')
                    ...
                    plt.plot(epochss, predict_list[0:predict_list.shape[0]], color='magenta', label='predict',
                             linewidth=1)
                    plt.plot(epochss, true_list[0:true_list.shape[0]], color='cyan', label='true', linewidth=1)
                    plt.legend(loc='upper right', fontsize=10)
                    plt.title('True_Predict')
                    plt.savefig(os.path.join('', 'true_predict'+ str(i) +'.jpg'))
                    i += 1
