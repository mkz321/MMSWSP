...
class MMSWSP(nn.Module):
    def __init__(self, args, data):
        ...
        for i in range(self.n_CNN):
            if i == 0:
                tmpconv = nn.Conv2d(1, self.hidC, kernel_size=(self.kernel_size, self.idim),
                                    padding=(self.kernel_size // 2, 0))
                # tmpconv = nn.Conv2d(1, self.hidC, kernel_size=(self.kernel_size, self.idim), stride=1, padding=1, groups=1)
                # tmpconv = nn.Sequential(
                #       nn.Conv2d(1, 1, kernel_size=(self.kernel_size, self.idim), stride=1, padding=(self.kernel_size//2, 0), groups=1),
                #       nn.Conv2d(1, self.hidC, kernel_size=1, stride=1, padding=0))
            else:
                tmpconv = nn.Conv2d(1, self.hidC, kernel_size=(self.kernel_size, self.hidC),
                                    padding=(self.kernel_size // 2, 0))
                # tmpconv = nn.Sequential(
                #     nn.Conv2d(1, 1, kernel_size=(self.kernel_size, self.hidC), stride=1, padding=(self.kernel_size//2, 0), groups=1),
                #     nn.Conv2d(1, self.hidC, kernel_size=1, stride=1, padding=0))f.kernel_size, self.hidC), padding=(self.kernel_size//2, 0))
    ...
    def forward(self, x):
        ...
        for i in range(self.collaborate_span * 2 + 1):
            if i == self.collaborate_span:
                #gmp2 = torch.nn.AdaptiveMaxPool2d(1)
                #target_result = gmp2(target_result)
